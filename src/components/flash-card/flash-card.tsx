...
export const FlashCard = forwardRef<HTMLDivElement, FlashCardProps>(
  (
    ...
    return (
      ...
      >
        <button
          className="relative h-72 w-full rounded-3xl border bg-card text-left text-card-foreground shadow-sm transition-transform focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          type="button"
        ...
        >
          <div
            className={cn(
              "absolute inset-0 rounded-3xl p-6 transition-all duration-300 [transform-style:preserve-3d]",
              isFlipped && "[transform:rotateY(180deg)]",
            )}
          >
            <div className="absolute inset-0 flex h-full flex-col justify-between rounded-3xl bg-gradient-to-br from-card via-card to-muted p-6 [backface-visibility:hidden]">
            ...
            <div className="absolute inset-0 flex h-full flex-col justify-between rounded-3xl bg-gradient-to-br from-lesson/15 via-card to-progress/15 p-6 [backface-visibility:hidden] [transform:rotateY(180deg)]">
              <div className="text-xs font-semibold uppercase tracking-[0.3em] text-muted-foreground">
                Back
              </div>
