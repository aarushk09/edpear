  ...
  ({ streak, goal = 7, days = defaultDays, className, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "rounded-3xl border bg-card p-5 text-card-foreground shadow-sm",
          "flex flex-col gap-5",
          className,
        )}
      ...
      >
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="rounded-full bg-quiz/10 p-3 text-quiz">
          ...
          <div className="flex gap-1">
            {goalSegments.map((filled, index) => (
              <span
                key={`goal-${index}`}
                aria-hidden="true"
                className={cn("h-2 flex-1 rounded-full", filled ? "bg-quiz" : "bg-muted")}
              />
            ))}
          </div>
        </div>
        <div className="grid grid-cols-7 gap-2" aria-label="Last seven days">
          {days.slice(0, 7).map((day) => (
            <div key={day.label} className="space-y-2 text-center">
              ...
              <div
                className={cn(
                  "mx-auto h-8 w-8 rounded-full border",
                  day.completed
                    ? "border-quiz bg-quiz text-primary-foreground"
