...
import type { StudyGroupMatch, StudyGroupFinderProps } from "./study-group-finder.types.js";

const DEFAULT_ITEMS: StudyGroupMatch[] = [
  {
    "id": "sg1",
    "title": "Algebra problem-solving pod",
    "subtitle": "EST · Tue/Thu 7:00 PM",
    "detail": "Small group focused on showing work clearly and checking reasoning together.",
    "status": "open",
    "tag": "math",
    "evidence": [
      "3 members",
  ...
  {
    "id": "sg2",
    "title": "AP Bio vocabulary sprint",
    "subtitle": "CST · Wed 6:30 PM",
    "detail": "Fast-paced retrieval practice with shared flashcards and short check-ins.",
    "status": "open",
    "tag": "science",
    "evidence": [
      "4 members",
      "Shared deck included"
    ],
    "score": 4
  },
  {
    "id": "sg3",
    "title": "Essay revision circle",
    "subtitle": "GMT · Sat 10:00 AM",
    "detail": "Peer accountability group for thesis, evidence, and structure feedback.",
    "status": "waitlist",
    "tag": "writing",
    "evidence": [
      "5 members",
...
export const StudyGroupFinder = forwardRef<HTMLDivElement, StudyGroupFinderProps>((props, ref) => {
  const {
    title = "Study Group Finder",
    subtitle = "Learner matching surface that organizes study partners by topic, timezone, and availability.",
    groups = DEFAULT_ITEMS,
    primaryActionLabel = "Join group",
    secondaryActionLabel = "Save match",
    disabled = false,
    onJoinGroup,
    onSaveGroup,
    className,
  ...
  const activeItem = useMemo(
    () => groups.find((entry) => entry.id === activeId) ?? groups[0] ?? null,
    [activeId, groups],
  );
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs sm:text-sm">
            ...
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              ...
              <p className="mt-1 font-semibold">{groups.filter((entry) => entry.status).length}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              <p className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Evidence</p>
              <p className="mt-1 font-semibold">{groups.flatMap((entry) => entry.evidence ?? []).length}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3 pt-1">
                <button
                  type="button"
                  disabled={disabled}
                  onClick={() => activeItem && onJoinGroup?.(activeItem)}
                ...
                <button
                  type="button"
                  disabled={disabled}
                  onClick={() => activeItem && onSaveGroup?.(activeItem)}
...
StudyGroupFinder.displayName = "StudyGroupFinder";
