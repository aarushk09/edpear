...
import type { PortfolioArtifact, PortfolioBuilderProps } from "./portfolio-builder.types.js";

const DEFAULT_ITEMS: PortfolioArtifact[] = [
  {
    "id": "pf1",
    "title": "Water quality investigation",
    "subtitle": "Science capstone",
    "detail": "Includes data collection photos, a final report, and a short reflection on revision decisions.",
    "status": "featured",
    "tag": "research",
    "evidence": [
      "Lab report",
  ...
  {
    "id": "pf2",
    "title": "Podcast script revision",
    "subtitle": "ELA project",
    "detail": "Shows how narration improved across three rounds of feedback and rehearsal.",
    "status": "ready",
    "tag": "communication",
    "evidence": [
      "Draft 1",
  ...
  {
    "id": "pf3",
    "title": "Community budget infographic",
    "subtitle": "Math application",
    "detail": "Combines ratio reasoning, visual design, and audience-friendly explanations.",
    "status": "ready",
    "tag": "applied math",
    "evidence": [
      "Poster export",
...
export const PortfolioBuilder = forwardRef<HTMLDivElement, PortfolioBuilderProps>((props, ref) => {
  const {
    title = "Portfolio Builder",
    subtitle = "Curated artifact gallery where learners pair project evidence with reflection and growth notes.",
    artifacts = DEFAULT_ITEMS,
    primaryActionLabel = "Feature",
  ...
  const activeItem = useMemo(
    () => artifacts.find((entry) => entry.id === activeId) ?? artifacts[0] ?? null,
    [activeId, artifacts],
  );
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs sm:text-sm">
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              ...
              <p className="mt-1 font-semibold">{artifacts.length}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              <p className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Signals</p>
              <p className="mt-1 font-semibold">{artifacts.filter((entry) => entry.status).length}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              <p className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Evidence</p>
              <p className="mt-1 font-semibold">{artifacts.flatMap((entry) => entry.evidence ?? []).length}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3 pt-1">
                <button
                  type="button"
                  disabled={disabled}
                  onClick={() => activeItem && onFeatureArtifact?.(activeItem)}
                ...
                <button
                  type="button"
                  disabled={disabled}
                  onClick={() => activeItem && onShareArtifact?.(activeItem)}
