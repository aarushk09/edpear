...
import type { BurnoutRiskSignal, BurnoutRiskIndicatorProps } from "./burnout-risk-indicator.types.js";

const DEFAULT_POINTS: BurnoutRiskSignal[] = [
  {
    "id": "br1",
    ...
    "current": 4,
    "target": 1,
    "detail": "Workload pressure is showing up first in missed deadlines."
  },
  {
    "id": "br2",
    "label": "Session drop-offs",
    "baseline": 2,
    "current": 5,
    "target": 2,
    "detail": "Learner exits long sessions earlier than usual this week."
  },
  {
    "id": "br3",
    "label": "Help requests",
    ...
    "current": 1,
    "target": 3,
    "detail": "A sudden drop in help-seeking can signal withdrawal rather than mastery."
  }
];

function maxValue(points: BurnoutRiskSignal[]) {
  return Math.max(...points.flatMap((point) => [point.baseline, point.current, point.target ?? 0]), 1);
}

export const BurnoutRiskIndicator = forwardRef<HTMLDivElement, BurnoutRiskIndicatorProps>((props, ref) => {
  const {
    title = "Burnout Risk Indicator",
    subtitle = "Engagement signal composite that flags overload trends before learners disengage.",
  ...
  return (
    <div
      ...
      data-slot="burnout-risk-indicator"
      {...rest}
    >
...
BurnoutRiskIndicator.displayName = "BurnoutRiskIndicator";
