...
import type { TranscriptCourseEntry, DigitalTranscriptProps } from "./digital-transcript.types.js";

const DEFAULT_ITEMS: TranscriptCourseEntry[] = [
  {
    "id": "dt1",
    "title": "Algebra I Foundations",
    "subtitle": "Completed · 0.5 credit",
    "detail": "Transcript includes final grade, attendance summary, and assessment evidence.",
    "status": "verified",
    "tag": "mathematics",
    "evidence": [
      "Grade A-",
  ...
  },
  {
    ...
    "subtitle": "Completed · 1.0 credit",
    "detail": "Shows mastery evidence and lab safety clearance on the transcript record.",
    "status": "verified",
    "tag": "science",
    "evidence": [
      "Grade B+",
  ...
  {
    "id": "dt3",
    "title": "Argument Writing Seminar",
    "subtitle": "In progress · 0.5 credit",
    "detail": "Current transcript entry reflects running grade and next verification checkpoint.",
    "status": "in progress",
    "tag": "ELA",
    "evidence": [
      "Midterm score 85"
...
export const DigitalTranscript = forwardRef<HTMLDivElement, DigitalTranscriptProps>((props, ref) => {
  const {
    title = "Digital Transcript",
    subtitle = "Verifiable course history view with completion evidence and a shareable signature surface.",
    courses = DEFAULT_ITEMS,
    primaryActionLabel = "View credential",
  ...
  const activeItem = useMemo(
    () => courses.find((entry) => entry.id === activeId) ?? courses[0] ?? null,
    [activeId, courses],
  );
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs sm:text-sm">
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              ...
              <p className="mt-1 font-semibold">{courses.length}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              <p className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Signals</p>
              <p className="mt-1 font-semibold">{courses.filter((entry) => entry.status).length}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              <p className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Evidence</p>
              <p className="mt-1 font-semibold">{courses.flatMap((entry) => entry.evidence ?? []).length}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3 pt-1">
                <button
                  type="button"
                  disabled={disabled}
                  onClick={() => activeItem && onViewCredential?.(activeItem)}
                ...
                <button
                  type="button"
                  disabled={disabled}
                  onClick={() => activeItem && onCopyShareLink?.(activeItem)}
...
DigitalTranscript.displayName = "DigitalTranscript";
