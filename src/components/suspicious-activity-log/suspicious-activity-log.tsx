...
import type { SuspiciousActivityEvent, SuspiciousActivityLogProps } from "./suspicious-activity-log.types.js";

const DEFAULT_ITEMS: SuspiciousActivityEvent[] = [
  {
    "id": "sa1",
    "title": "Paste event detected",
    "subtitle": "11:04 AM · short answer",
    "detail": "The learner pasted a 142-character response into a free-response field.",
    "status": "flag",
    "tag": "paste",
    "evidence": [
      "Question 6",
  ...
  {
    "id": "sa2",
    "title": "Focus lost twice",
    "subtitle": "11:09 AM · browser blur",
    "detail": "Two blur events occurred within the same 30-second window.",
    "status": "flag",
    "tag": "focus",
    "evidence": [
      "Blur 11:09:02",
  ...
  {
    "id": "sa3",
    "title": "Idle gap exceeded threshold",
    "subtitle": "11:17 AM · inactivity",
    "detail": "No keyboard or mouse activity for four minutes during section B.",
    "status": "watch",
    "tag": "idle",
    "evidence": [
      "Idle 4m 03s"
...
export const SuspiciousActivityLog = forwardRef<HTMLDivElement, SuspiciousActivityLogProps>((props, ref) => {
  const {
    title = "Suspicious Activity Log",
    subtitle = "Timestamped audit trail for paste events, focus losses, idle gaps, and integrity review notes.",
    events = DEFAULT_ITEMS,
  ...
  const activeItem = useMemo(
    () => events.find((entry) => entry.id === activeId) ?? events[0] ?? null,
    [activeId, events],
  );
  ...
  return (
    <div
      ...
      data-slot="suspicious-activity-log"
      {...rest}
    >
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs sm:text-sm">
            ...
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              ...
              <p className="mt-1 font-semibold">{events.filter((entry) => entry.status).length}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              <p className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Evidence</p>
              <p className="mt-1 font-semibold">{events.flatMap((entry) => entry.evidence ?? []).length}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3 pt-1">
                <button
                  type="button"
                  disabled={disabled}
                  onClick={() => activeItem && onReviewEvent?.(activeItem)}
                ...
                <button
                  type="button"
                  disabled={disabled}
                  onClick={() => activeItem && onResolveEvent?.(activeItem)}
...
SuspiciousActivityLog.displayName = "SuspiciousActivityLog";
