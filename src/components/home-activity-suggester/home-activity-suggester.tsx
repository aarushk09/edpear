...
import type { HomeActivitySuggestion, HomeActivitySuggesterProps } from "./home-activity-suggester.types.js";

const DEFAULT_ITEMS: HomeActivitySuggestion[] = [
  {
    "id": "h1",
    "title": "Kitchen ratio scavenger hunt",
    "subtitle": "15 minutes · Math",
    "detail": "Compare recipe quantities and ask the learner to scale them up or down.",
    "status": "easy",
    "tag": "family",
    "evidence": [
      "No prep",
  ...
  {
    "id": "h2",
    "title": "Claim and evidence dinner chat",
    "subtitle": "10 minutes · ELA",
    "detail": "Ask everyone to make a claim and back it with one concrete example.",
    "status": "easy",
    "tag": "conversation",
    "evidence": [
  ...
  {
    "id": "h3",
    "title": "Neighborhood biodiversity walk",
    "subtitle": "20 minutes · Science",
    "detail": "Spot and classify living things, then discuss what habitats they need.",
    "status": "medium",
    "tag": "observation",
    "evidence": [
      "No screens required"
...
export const HomeActivitySuggester = forwardRef<HTMLDivElement, HomeActivitySuggesterProps>((props, ref) => {
  const {
    title = "Home Activity Suggester",
    subtitle = "Curated offline exercises families can do together to reinforce classroom learning.",
    suggestions = DEFAULT_ITEMS,
    primaryActionLabel = "Assign tonight",
  ...
  const [activeId, setActiveId] = useState(suggestions[0]?.id ?? null);
  const activeItem = useMemo(
    () => suggestions.find((entry) => entry.id === activeId) ?? suggestions[0] ?? null,
  ...
  return (
    <div
      ...
      data-slot="home-activity-suggester"
      {...rest}
    >
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs sm:text-sm">
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              ...
              <p className="mt-1 font-semibold">{suggestions.length}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              <p className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Signals</p>
              <p className="mt-1 font-semibold">{suggestions.filter((entry) => entry.status).length}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              <p className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Evidence</p>
              <p className="mt-1 font-semibold">{suggestions.flatMap((entry) => entry.evidence ?? []).length}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3 pt-1">
                <button
                  type="button"
                  disabled={disabled}
                  onClick={() => activeItem && onAssignSuggestion?.(activeItem)}
                ...
                <button
                  type="button"
                  disabled={disabled}
                  onClick={() => activeItem && onSaveSuggestion?.(activeItem)}
...
HomeActivitySuggester.displayName = "HomeActivitySuggester";
