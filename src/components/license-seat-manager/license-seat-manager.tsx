...
import type { LicenseSeatPlan, LicenseSeatManagerProps } from "./license-seat-manager.types.js";

const DEFAULT_POINTS: LicenseSeatPlan[] = [
  {
    "id": "ls1",
    "label": "District math license",
    "baseline": 420,
    "current": 470,
    "target": 500,
    "detail": "Usage is healthy, but only 30 seats remain before the next renewal cycle."
  },
  {
    "id": "ls2",
    "label": "Science intervention bundle",
    "baseline": 118,
    "current": 140,
    "target": 150,
    "detail": "Seat assignment increased after the midyear onboarding push."
  },
  {
    "id": "ls3",
    "label": "ELA tutoring add-on",
    "baseline": 62,
    "current": 75,
    "target": 90,
    "detail": "Room remains for two more cohorts without a contract change."
  }
];

function maxValue(points: LicenseSeatPlan[]) {
  return Math.max(...points.flatMap((point) => [point.baseline, point.current, point.target ?? 0]), 1);
}

export const LicenseSeatManager = forwardRef<HTMLDivElement, LicenseSeatManagerProps>((props, ref) => {
  const {
    title = "License Seat Manager",
    subtitle = "Org-level seat dashboard that compares allocation, usage, and remaining capacity at a glance.",
    plans = DEFAULT_POINTS,
    baselineLabel = "Used",
  ...
  const peak = useMemo(() => maxValue(plans), [plans]);
  const activePoint = useMemo(
    () => plans.find((point) => point.id === activeId) ?? plans[0] ?? null,
    [activeId, plans],
  );
  const deltaAverage = useMemo(() => {
    if (!plans.length) return 0;
    const total = plans.reduce((sum, point) => sum + (point.current - point.baseline), 0);
    return Math.round((total / plans.length) * 10) / 10;
  }, [plans]);
  ...
  return (
    <div
      ...
      data-slot="license-seat-manager"
      {...rest}
    >
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs sm:text-sm">
            ...
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              ...
              <p className="mt-1 font-semibold">{plans.length}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              <p className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Targets</p>
              <p className="mt-1 font-semibold">{plans.filter((point) => point.target !== undefined).length}</p>
            </div>
          </div>
        </div>
      </div>
...
LicenseSeatManager.displayName = "LicenseSeatManager";
