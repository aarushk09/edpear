
# ConceptMapBuilder

Freeform node-link planning surface for learner-generated knowledge maps.

## Installation

```bash
npx edpear add concept-map-builder
```

## Basic Usage

```tsx
import { ConceptMapBuilder } from "edpear";

<ConceptMapBuilder />
```

## Props

| Name | Type | Default | Description |
| --- | --- | --- | --- |
| `title` | `string` | `Concept Map Builder` | Heading text for the component card |
| `subtitle` | `string` | Built-in copy | Supporting product guidance under the title |
| `nodes` | `ConceptMapNode[]` | Built-in sample data | Replace with your own product data |
| `disabled` | `boolean` | `false` | Disables interactive controls while preserving layout |
| `onSaveNodes` | `(items: ConceptMapNode[]) => void` | `undefined` | Called when the current draft is saved |

## Accessibility

- Keeps all interactive elements keyboard reachable with visible focus treatment.
- Uses plain language section headings so assistive technologies can scan the component structure quickly.
- Preserves readable contrast and spacing in both dense dashboard layouts and embedded doc previews.
