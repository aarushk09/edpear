
# SentenceConstructor

Word-tile practice surface for assembling grammatically correct sentence order.

## Installation

```bash
npx edpear add sentence-constructor
```

## Basic Usage

```tsx
import { SentenceConstructor } from "edpear";

<SentenceConstructor />
```

## Props

| Name | Type | Default | Description |
| --- | --- | --- | --- |
| `title` | `string` | `Sentence Constructor` | Heading text for the component card |
| `subtitle` | `string` | Built-in copy | Supporting product guidance under the title |
| `tiles` | `SentenceTile[]` | Built-in sample data | Replace with your own product data |
| `disabled` | `boolean` | `false` | Disables interactive controls while preserving layout |
| `onSaveTiles` | `(items: SentenceTile[]) => void` | `undefined` | Called when the current draft is saved |

## Accessibility

- Keeps all interactive elements keyboard reachable with visible focus treatment.
- Uses plain language section headings so assistive technologies can scan the component structure quickly.
- Preserves readable contrast and spacing in both dense dashboard layouts and embedded doc previews.
