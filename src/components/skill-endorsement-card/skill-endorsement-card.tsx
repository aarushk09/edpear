...
import type { SkillEndorsement, SkillEndorsementCardProps } from "./skill-endorsement-card.types.js";

const DEFAULT_ITEMS: SkillEndorsement[] = [
  {
    "id": "se1",
    "title": "Scientific data interpretation",
    "subtitle": "Signed by Dr. Lopez",
    "detail": "Endorsement highlights evidence from the capstone lab and discussion defense.",
    "status": "issued",
    "tag": "science",
    "evidence": [
      "Rubric 4/4",
  ...
  },
  {
    ...
    "subtitle": "Signed by Ms. Rivera",
    "detail": "Card recognizes leadership during weekly algebra tutoring groups.",
    "status": "issued",
    "tag": "collaboration",
    "evidence": [
      "Peer feedback",
  ...
  },
  {
    ...
    "detail": "Awaiting final sign-off after portfolio commentary is updated.",
    "status": "pending",
    "tag": "writing",
    "evidence": [
      "Portfolio note in draft"
    ],
    "score": 3
  }
];

export const SkillEndorsementCard = forwardRef<HTMLDivElement, SkillEndorsementCardProps>((props, ref) => {
  const {
    title = "Skill Endorsement Card",
    subtitle = "Instructor-signed competency card for showcasing validated skills on learner profiles.",
    endorsements = DEFAULT_ITEMS,
    primaryActionLabel = "Share",
    secondaryActionLabel = "Request revision",
    disabled = false,
    onShareEndorsement,
    onRequestRevision,
    className,
  ...
  const [activeId, setActiveId] = useState(endorsements[0]?.id ?? null);
  const activeItem = useMemo(
    () => endorsements.find((entry) => entry.id === activeId) ?? endorsements[0] ?? null,
    [activeId, endorsements],
  );
  ...
  return (
    <div
      ...
      data-slot="skill-endorsement-card"
      {...rest}
    >
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs sm:text-sm">
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              ...
              <p className="mt-1 font-semibold">{endorsements.length}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              <p className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Signals</p>
              <p className="mt-1 font-semibold">{endorsements.filter((entry) => entry.status).length}</p>
            </div>
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              <p className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Evidence</p>
              <p className="mt-1 font-semibold">{endorsements.flatMap((entry) => entry.evidence ?? []).length}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3 pt-1">
                <button
                  type="button"
                  disabled={disabled}
                  onClick={() => activeItem && onShareEndorsement?.(activeItem)}
                ...
                <button
                  type="button"
                  disabled={disabled}
                  onClick={() => activeItem && onRequestRevision?.(activeItem)}
...
SkillEndorsementCard.displayName = "SkillEndorsementCard";
