...
import { Megaphone, ArrowDown, ArrowUp, GripVertical, Plus, Save } from "lucide-react";
import { forwardRef, useState } from "react";

import { cn } from "../../lib/cn.js";
import type { ScheduledAnnouncementDraft, AnnouncementSchedulerProps } from "./announcement-scheduler.types.js";

const DEFAULT_ITEMS: ScheduledAnnouncementDraft[] = [
  {
    "id": "an1",
    "title": "Family progress update",
    "detail": "Send Thursday at 6:00 PM local time with digest and attendance highlights.",
    "tag": "scheduled"
  },
  {
    "id": "an2",
    "title": "Assessment reminder",
    "detail": "Send one hour before the benchmark window opens across all campuses.",
    "tag": "timed"
  },
  {
    "id": "an3",
    "title": "Club signup celebration",
    "detail": "Queue after school dismissal so families see the update together.",
    "tag": "community"
  }
];
...
export const AnnouncementScheduler = forwardRef<HTMLDivElement, AnnouncementSchedulerProps>((props, ref) => {
  const {
    title = "Announcement Scheduler",
    subtitle = "Rich-text announcement composer with timezone-aware scheduling and send-window visibility.",
    announcements = DEFAULT_ITEMS,
  ...
  return (
    <div
      ...
      data-slot="announcement-scheduler"
      {...rest}
    >
        ...
        <aside className="rounded-2xl border border-border bg-muted/10 p-4 sm:p-5">
          <div className="space-y-3">
            ...
            <button
              type="button"
              disabled={disabled}
              onClick={() => onSaveAnnouncements?.(items)}
...
AnnouncementScheduler.displayName = "AnnouncementScheduler";
