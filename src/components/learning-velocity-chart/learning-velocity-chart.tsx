...
import type { LearningVelocityPoint, LearningVelocityChartProps } from "./learning-velocity-chart.types.js";

const DEFAULT_POINTS: LearningVelocityPoint[] = [
  {
    "id": "w1",
    "label": "Week 1",
    "baseline": 3,
    "current": 4,
    "target": 5,
    "detail": "Strong start during onboarding module."
  },
  {
    "id": "w2",
    "label": "Week 2",
    ...
    "current": 6,
    "target": 5,
    "detail": "Practice volume pushed velocity above target."
  },
  {
    "id": "w3",
    "label": "Week 3",
    ...
    "current": 5,
    "target": 6,
    "detail": "A harder unit slowed acquisition slightly."
  }
];

function maxValue(points: LearningVelocityPoint[]) {
  return Math.max(...points.flatMap((point) => [point.baseline, point.current, point.target ?? 0]), 1);
}

export const LearningVelocityChart = forwardRef<HTMLDivElement, LearningVelocityChartProps>((props, ref) => {
  const {
    title = "Learning Velocity Chart",
    subtitle = "Time-series view of how quickly concepts are being acquired across a course.",
  ...
  const peak = useMemo(() => maxValue(points), [points]);
  ...
  const deltaAverage = useMemo(() => {
    ...
    const total = points.reduce((sum, point) => sum + (point.current - point.baseline), 0);
  ...
  return (
    <div
      ...
      data-slot="learning-velocity-chart"
      {...rest}
    >
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs sm:text-sm">
            ...
            <div className="rounded-xl border border-border bg-muted/20 px-3 py-2">
              ...
              <p className="mt-1 font-semibold">{points.filter((point) => point.target !== undefined).length}</p>
            </div>
          </div>
        </div>
      </div>
...
LearningVelocityChart.displayName = "LearningVelocityChart";
