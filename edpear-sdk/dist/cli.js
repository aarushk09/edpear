...
class EdPearCLI {
    ...
    async login() {
        ...
        try {
            ...
            console.log(chalk_1.default.gray('You will receive a 6-digit verification code via email.'));
    ...
    getApiKey() {
        ...
        if (storedKeys && storedKeys.length > 0)
            return storedKeys[0].key;
        console.error(chalk_1.default.red('❌ No API key found.'));
        console.error(chalk_1.default.gray('Run "edpear generate-key" or set EDPEAR_API_KEY in your environment.'));
        process.exit(1);
    }
    ...
    async mathDebug(imagePath) {
        ...
        try {
            const client = new index_1.EdPearClient({ apiKey: this.getApiKey() });
