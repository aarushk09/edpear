...
/**
 * Sends an OTP email to the user using Brevo.
...
export async function sendOTPEmail(email: string, otp: string, userName?: string) {
  try {
    // If no Brevo API key is configured, log the OTP for development
    if (!process.env.BREVO_API_KEY) {
      console.log('📧 Brevo API Key not found. OTP Email (Development Mode):');
      console.log(`   To: ${email}`);
      console.log(`   OTP: ${otp}`);
      console.log('   ⚠️  Configure BREVO_API_KEY in .env.local for production email delivery');
      return { success: true, message: 'OTP logged to console (BREVO_API_KEY missing)' };
    }

    const sendSmtpEmail = new Brevo.SendSmtpEmail();

    sendSmtpEmail.subject = "EdPear CLI Authentication Code";
    sendSmtpEmail.to = [{ email: email, name: userName || "EdPear User" }];
    sendSmtpEmail.sender = { 
      name: "EdPear", 
      email: process.env.EMAIL_FROM || "noreply@edpear.com" 
    };
...
© ${new Date().getFullYear()} EdPear Platform. All rights reserved.
    ...
    // In development/test environments, we might want to still see the OTP even if Brevo fails
